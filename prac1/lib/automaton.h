...
class AutomatonT {
 private:
  struct Node {
    ...
    void AddEdge(size_t to, const std::string& word) { go[word].insert(to); }
  };
 ...
 protected:
  std::set<std::string> GetWordsCycle(std::set<std::string> from,
                                      std::set<std::string>& read, size_t index,
                                      int cur_depth, int depth) {
    if (cur_depth >= depth) return from;
    ...
    for (auto it : m_nodes_[index].go) {
      ...
      for (auto it1 : it.second) {
        std::set<std::string> cp1 = GetWordsCycle(
            cp, read, it1,
            cur_depth + (it.first == "#" ? 0 : it.first.length()), depth);
...
class AutomatonTransform {
      ...
      for (auto it1 : it.second) {
        if (it.first == "#") {
          ...
          if (visited.find(it1) == visited.end()) {
            std::set<size_t> temp = HelperClosureDFS(it1, visited, closure);
 ...
 public:
  ...
  AutomatonT<kDeterministic> GetDFAThompson(
      AutomatonT<kNondeterministic>& source) {
  ...
  AutomatonT<kComplete> Minimal(AutomatonT<kDeterministic>& source) {
    ...
    for (size_t i = 0; i < source.m_nodes_.size(); ++i) {
      ...
      for (auto it : temp->go) {
        for (size_t ind : it.second) {
          ...
          cur.letters_q.push_back((source.m_nodes_[ind].type == kTerminal) ? 0
                                                                           : 1);
          ++cur.letters_sz;
        }
      }
    ...
    for (size_t i = 0; i < equiv.size(); ++i) {
      ...
      for (size_t j = 0; j < equiv[i].letters_q.size(); ++j) {
        minim.AddEdge(equiv[i].q, equiv[i].letters_q[j], equiv[i].letters[j]);
      }
    }
    return minim;
  }
...
class AutomatonBuilder {
 ...
 public:
  AutomatonT<kDeterministic> BuildDFAFromNFA(
    ...
    AutomatonT<kDeterministic> b = abstract_transformer.GetDFAThompson(source);
    return b;
  }
